...
\newcommand{\tanta}{
	\mathord{
		\tikz[scale=0.3, baseline={(0,-0.1)}]{
			\draw plot[smooth] coordinates{
				(-0.4,0) (-0.35,0.23) (0.03,0.8)
			};
			
			\draw plot[smooth] coordinates{
				(0.4,0) (0.35,-0.23) (-0.03,-0.8)
			};
			\draw (-0.2,0) circle [radius=0.2];
			\draw (0.2,0) circle [radius=0.2];
...
\begin{document}
	...
	\vspace{1em}
	\[ \mo^{c_0}_{\xm m_0 \xc u}(k) = \mo^{m_0}_{\xm c_0 \xc k}(u)\]
	
	\vspace{1em}
	\[ \mo^{1}_{\xc d}(k) = \mo^{1}_{\xc \frac{k+1}{d}}(k) \]
	
	\vspace{1em}
	\[ \mo^1_{\xc d}(p) \he \mo^1_{\xc \tfrac{p-1}{d}}(p) \]
	
		\[ \mo^{c_0}_{X \xm m_0 \xc u}(k) \]
	\[ \tanta \]
	\[ c_0 \]
	
	\[ \mo^{1}_{\xc u}(u^v + 1)=2v \]
	
	\[ w_n = e^{\frac{2\pi j}{n}} = cos(\frac{2\pi}{n}+j sin(\frac{2\pi}{n})) \]
	...
	\[ \mo^{E}_{\xc U}(\tanta) = \mo^{E}_{\xc U} = lcm(n_1, n_2, ... , n_i)\]
	...
	\begin{pmatrix}
		...
		c_{n^2-n+1} & c_{n^2-n+2} & \cdots & c_{n^2}
	\end{pmatrix}
	\]
	...
	\[ \mo^{C_o}_{\xc U}(k) = lcm(\mo^{u\sum^{n}_{i=1}c_i}_{\xc nu}(k),\mo^{u\sum^{2n}_{i=n+1}c_i}_{\xc nu}(k),...,\mo^{u\sum^{n^2}_{i=n^2-n+1}c_i}_{\xc nu}(k))\]
